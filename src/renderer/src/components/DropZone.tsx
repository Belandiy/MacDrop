...
import { UploadCloud, FileCheck, Copy, Ban, AlertCircle, Laptop, Monitor, Radio, Folder, FileUp } from 'lucide-react';
...
interface DropZoneProps {
  ...
  onChooseFolder?: (targetDeviceId?: string) => void;
        ...
        <div className="flex items-center gap-2 mt-1">
          {onChooseFiles && (
            <button
              ...
              className="text-xs bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 active:scale-95 text-white font-semibold px-3.5 py-2 rounded-xl shadow-md shadow-indigo-600/30 transition-all flex items-center gap-1.5"
            >
              <FileUp className="w-3.5 h-3.5" />
              <span>{targetName ? `Файлы для ${targetName}` : 'Выбрать файлы'}</span>
            </button>
          )}
          ...
          {onChooseFolder && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onChooseFolder(targetDevice?.id);
              }}
              className="text-xs bg-white/10 hover:bg-white/15 active:scale-95 text-indigo-200 hover:text-white border border-indigo-400/30 font-semibold px-3.5 py-2 rounded-xl transition-all flex items-center gap-1.5"
